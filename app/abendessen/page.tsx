    ...
    <>
          ...
          {DINNER_MENU.map((cat) => (
            ...
            >
              ...
              <div className="mb-12">
                ...
                {cat.hint && (
                  <p className="mt-3 font-display italic text-stone-500">
                    {cat.hint}
                  </p>
                )}
              </div>
                ...
                {cat.items.map((dish) => (
                  <li key={dish.name}>
                    ...
                    {dish.options?.map((opt) => (
                      ...
                      >
                        + {opt.label}{" "}
                        <span className="text-stone-500">{opt.price}</span>
                      </p>
                    ))}
                  </li>
                ))}
      ...
      <section className="bg-stone-soft border-y border-stone-200">
        <div className="mx-auto max-w-3xl px-6 md:px-10 py-20 md:py-24 reveal">
          <div className="flex items-baseline gap-5 mb-8">
            <h2 className="font-display italic text-stone-400 text-lg tracking-wide">
              Beim Burger
            </h2>
            <span aria-hidden className="flex-1 h-px bg-stone-300" />
          </div>
          <p className="font-display italic text-stone-600 leading-relaxed mb-10 max-w-xl">
          ...
          <div className="grid sm:grid-cols-3 gap-10 text-sm">
            <div>
              ...
              <ul className="space-y-1.5 text-waldgruen font-display">
              ...
              <ul className="space-y-1.5 text-waldgruen font-display">
                {BURGER_CHOICES.extras.map((e) => (
                  <li key={e.label} className="flex justify-between gap-3">
                    <span>{e.label}</span>
                    <span className="text-stone-500">{e.price}</span>
                  </li>
                ))}
              </ul>
            </div>
      ...
      <section className="bg-white">
        <div className="mx-auto max-w-3xl px-6 md:px-10 py-20 md:py-28 text-center reveal">
          ...
          <h2 className="mt-7 text-3xl md:text-4xl font-display font-normal leading-tight tracking-tight text-waldgruen">
